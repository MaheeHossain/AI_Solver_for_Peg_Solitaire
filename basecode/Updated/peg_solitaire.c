...
/* Set some sorta buffer if enabled? */
void setBufferedInput(bool enable) {
	static bool enabled = true;
	static struct termios old; //time?
	struct termios new;

	if (enable && !enabled) {
		// restore the former settings
		tcsetattr(STDIN_FILENO,TCSANOW,&old); // In termios -> termiWin
		// set the new state
		enabled = true;
...
/* Message to print out if the correct inputs aren't given */
void print_usage(){
...
int main(int argc, char *argv[]) {
	...
	/* Checks what level (layout) it's running */
	int layout = 0; // (default: 0)
	sscanf (argv[1],"%d",&layout);

	/* Checks if its an AI run game or not */
	ai_run = false; // (default: false)
	if(  argc > 2 && strcmp(argv[2],"AI")==0 )
		ai_run = true;
	
	/* Checks the 'budget' of the game */
	budget = 1000000; //(default: 1M)
	if( argc > 3 )
		sscanf (argv[3],"%d",&budget);
	
	/* Checks whether it should show the AI's solution */
	...
	signal(SIGINT, signal_callback_handler); // Terminates the program

	// initialize board
	initialize(board,layout);  //utils.h
	memcpy(&solution[0],board,sizeof(*board));
	drawBoard(board); //utils.h
	...
	if( ai_run ){ 
		...
		// AI ALGORITHM CALL
		find_solution( board ); // ai.h

		clock_t end = clock();
     	double cpu_time_used = ((double) (end - start)) / CLOCKS_PER_SEC;

		// Play the animation for the solution
		if( show_solution ) play_solution(); //utils.h
	...
	/* HUMAN PLAYER */
		...
		while (true) {
			c=getchar();
			move = (*board).selected;
			// See what way the player wants to move, and move that way
			switch(c) {
				case 97:	// 'a' key
				case 104:	// 'h' key
				case 68:	// left arrow
